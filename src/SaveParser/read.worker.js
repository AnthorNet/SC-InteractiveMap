...
self.addEventListener(
  ...
  ({ data: { cmd, ...options } }) => {
    switch (cmd) {
      case "run":
        new SaveParser_Read_Worker(options);
        break;
    }
  },
  false
...
class SaveParser_Read_Worker
{
    ...
    {
        this.result.header                      = {};
        this.result.header.saveHeaderType       = this.readInt();
        this.result.header.saveVersion          = this.readInt();
        this.result.header.buildVersion         = this.readInt();
        this.result.header.mapName              = this.readString();
        this.result.header.mapOptions           = this.readString();
        this.result.header.sessionName          = this.readString();
        this.result.header.playDurationSeconds  = this.readInt();
        this.result.header.saveDateTime         = this.readLong();
        this.result.header.sessionVisibility    = this.readByte();

        if(this.result.header.saveHeaderType >= 7)
        {
            this.result.header.fEditorObjectVersion = this.readInt();
        }
        if(this.result.header.saveHeaderType >= 8)
        {
            this.result.header.modMetadata      = this.readString();
            this.result.header.isModdedSave     = this.readInt();
        }

        console.log(this.result.header);
    ...
    parseObjects()
    {
        ...
        if(this.result.header.saveVersion >= 21)
        {
    ...
    parseCompressedObjectsV21()
    {
        ...
        this.result.PACKAGE_FILE_TAG    = null;
        this.result.maxChunkSize        = null;
        this.currentChunks                  = [];
    ...
    inflateChunks()
    {
        while(this.handledByte < this.maxByte)
        {
            ...
            if(this.result.PACKAGE_FILE_TAG === null)
            {
                //this.result.PACKAGE_FILE_TAG = chunkHeader.getBigInt64(0, true);
                this.result.PACKAGE_FILE_TAG = chunkHeader.getUint32(0, true);
                //console.log(chunkHeader.getBigInt64(0, true), chunkHeader.getUint32(0, true));
            }
            if(this.result.maxChunkSize === null)
            {
                this.result.maxChunkSize = chunkHeader.getUint32(8, true);
            }
            ...
            postMessage({
              currentPercentage: currentPercentage * 0.4,
              message: 'Inflating save game (' + currentPercentage + '%)...',
            });
        }
    ...
    parseObjectsV5()
    {
        ...
        let entitiesToObjects           = [];
            ...
            postMessage({
              messageTranslateNumber: ['MAP\\SAVEPARSER\\Parsing %1$s objects...', countObjects],
            });
            ...
            let countEntities   = this.readInt();
                ...
                postMessage({
                  messageTranslateNumber: ['MAP\\SAVEPARSER\\Parsing %1$s entities...', countEntities],
                });
            ...
            this.result.collectables   = [];
                let countCollected  = this.readInt();
                    console.log('Parsing: ' + countCollected + ' collectables...');
                    postMessage({
                      messageTranslateNumber: ['MAP\\SAVEPARSER\\Parsing %1$s collectables...', countCollected],
                    });

                  for(let i = 0; i < countCollected; i++)
                  {
                      this.result.collectables.push(this.readObjectProperty({}));
                  }
    ...
    readEntityV5(objectKey)
    {
        ...
        if(this.result.objects[objectKey].type === 1)
        {
            this.result.objects[objectKey].entity = this.readObjectProperty({});

            let countChild  = this.readInt();
                if(countChild > 0)
                {
                    this.result.objects[objectKey].children = [];

                    for(let i = 0; i < countChild; i++)
                    {
                        this.result.objects[objectKey].children.push(this.readObjectProperty({}));
                    }
                }
        }
        ...
        {
            this.result.objects[objectKey].shouldBeNulled = true;
            return;
        }

        // Read properties
        this.result.objects[objectKey].properties       = [];
        while(true)
        {
            let property = this.readPropertyV5();
                ...
                this.result.objects[objectKey].properties.push(property);
        }

        // Read Conveyor missing bytes
        if(
                this.result.objects[objectKey].className.includes('/Build_ConveyorBeltMk')
             || this.result.objects[objectKey].className.includes('/Build_ConveyorLiftMk')
             // MODS
             || this.result.objects[objectKey].className.startsWith('/Conveyors_Mod/Build_BeltMk')
             || this.result.objects[objectKey].className.startsWith('/Conveyors_Mod/Build_LiftMk')
             || this.result.objects[objectKey].className.startsWith('/Game/CoveredConveyor')
             || this.result.objects[objectKey].className.startsWith('/CoveredConveyor/')
             || this.result.objects[objectKey].className.startsWith('/UltraFastLogistics/Buildable/build_conveyorbeltMK')
             || this.result.objects[objectKey].className.startsWith('/FlexSplines/Conveyor/Build_Belt')
        )
        {
            this.result.objects[objectKey].extra    = {count: this.readInt(), items: []};
            let itemsLength                             = this.readInt();
                for(let i = 0; i < itemsLength; i++)
                {
                    ...
                    this.result.objects[objectKey].extra.items.push(currentItem);
                }
        }
        else
        {
            // Extra processing
            switch(this.result.objects[objectKey].className)
            {
                ...
                case '/Game/FactoryGame/-Shared/Blueprint/BP_GameMode.BP_GameMode_C':
                    this.result.objects[objectKey].extra    = {count: this.readInt(), game: []};
                    let gameLength                              = this.readInt();

                    for(let i = 0; i < gameLength; i++)
                    {
                        this.result.objects[objectKey].extra.game.push(this.readObjectProperty({}));

                        if(i === 0 && this.result.objects[objectKey].className === '/Game/FactoryGame/-Shared/Blueprint/BP_GameState.BP_GameState_C')
                        {
                            this.result.playerHostPathName  = this.result.objects[objectKey].extra.game[0].pathName;
                        }
                    }
                ...
                case '/Game/FactoryGame/Character/Player/BP_PlayerState.BP_PlayerState_C':
                    let missingPlayerState                          = (startByte + entityLength) - this.currentByte;
                        this.result.objects[objectKey].missing  = this.readHex(missingPlayerState);
                                ...
                                switch(playerType)
                                {
                                    case 248: // EOS
                                        ...
                                        let eosStr                                      = this.readString().split('|');
                                            this.result.objects[objectKey].eosId    = eosStr[0];
                                        break;
                                    case 25: // Steam
                                        ...
                                        this.result.objects[objectKey].steamId = steamHex.replace(/^0+/, '');
                                        break;
                                    case 17: // Old EOS
                                        ...
                                        this.result.objects[objectKey].eosId = epicHex.replace(/^0+/, '');
                                        break;
                                    case 8: // ???
                                        this.result.objects[objectKey].platformId = this.readString();
                                        break;
                                    case 3: // Offline
                                        break;
                                    default:
                                        postMessage({
                                          alert: 'Something went wrong while we were trying to parse your save game... Please try to contact us on Twitter or Discord!'
                                        });
                                        if(typeof Sentry !== 'undefined')
                                        {
                                            Sentry.setContext('BP_PlayerState_C', this.result.objects[objectKey]);
                                            Sentry.setContext('playerType', playerType);
                                        }
                                        console.log(playerType, this.result.objects[objectKey]);
                ...
                case '/Game/FactoryGame/Buildable/Factory/DroneStation/BP_DroneTransport.BP_DroneTransport_C':
                    let missingDrone                                = (startByte + entityLength) - this.currentByte;
                        this.result.objects[objectKey].missing  = this.readHex(missingDrone);

                    break;
                case '/Game/FactoryGame/-Shared/Blueprint/BP_CircuitSubsystem.BP_CircuitSubsystem_C':
                    this.result.objects[objectKey].extra    = {count: this.readInt(), circuits: []};
                    let circuitsLength                          = this.readInt();

                        for(let i = 0; i < circuitsLength; i++)
                        {
                            this.result.objects[objectKey].extra.circuits.push({
                ...
                case '/AB_CableMod/Visuals3/Build_AB-PLPaintable.Build_AB-PLPaintable_C':
                    this.result.objects[objectKey].extra        = {
                        count   : this.readInt(),
                ...
                case '/Game/FactoryGame/Buildable/Vehicle/Train/Wagon/BP_FreightWagon.BP_FreightWagon_C':
                    this.result.objects[objectKey].extra    = {count: this.readInt(), objects: []};
                    let trainLength                             = this.readInt();
                        for(let i = 0; i < trainLength; i++)
                        {
                            this.result.objects[objectKey].extra.objects.push({
                                name   : this.readString(),
                                unk    : this.readHex(53)
                            });
                        }

                    this.result.objects[objectKey].extra.previous   = this.readObjectProperty({});
                    this.result.objects[objectKey].extra.next       = this.readObjectProperty({});
                    break;
                ...
                default:
                    let missingBytes = (startByte + entityLength) - this.currentByte;
                        if(missingBytes > 4)
                        {
                            this.result.objects[objectKey].missing = this.readHex(missingBytes); // TODO
                            console.log('MISSING ' + missingBytes + '  BYTES', this.result.objects[objectKey]);
                        }
                        else
                        {
                            this.skipBytes(4);
    ...
    readTextProperty(currentProperty)
    {
        ...
        switch(currentProperty.historyType)
        {
            ...
            case 255:
                ...
                if(this.result.header.buildVersion >= 140822)
                {
