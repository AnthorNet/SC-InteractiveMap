...
self.addEventListener(
    ...
    ({ data: { cmd, ...options } }) => {
        switch (cmd) {
        case "run":
            new SaveParser_Write_Worker(options);
            break;
        }
    },
    false
...
class SaveParser_Write_Worker
{
        ...
        let countObjects = objectsKeys.length;
            for(let i = 0; i < countObjects; i++)
            {
                ...
                if(this.saveBinary.length >= this.maxChunkSize)
                {
                    ...
                    postMessage({
                        currentPercentage: (i / countObjects * 100) * 0.48,
                        messageTranslateNumberNumber: ['MAP\\SAVEPARSER\\Compiling %1$s/%2$s objects...', i, countObjects],
                    });
                }
            }
    ...
    generateEntitiesChunks(objectsKeys)
    {
        let countObjects = objectsKeys.length;
            for(let i = 0; i < countObjects; i++)
            {
                ...
                if(this.saveBinary.length >= this.maxChunkSize)
                {
                    ...
                    postMessage({
                        currentPercentage: 48 + (i / countObjects * 100) * 0.48,
                        messageTranslateNumberNumber: ['MAP\\SAVEPARSER\\Compiling %1$s/%2$s entities...', i, countObjects],
                    });
                }
            }
        ...
        return this.generateCollectablesChunks(countCollectables);
    }

    generateCollectablesChunks(countCollectables)
    {
        postMessage({
            messageTranslateNumber: ['MAP\\SAVEPARSER\\Compiling %1$s collectables...', countCollectables],
        });
    ...
    {
        // postMessage({
        //     message: 'Deflate chunk ' + this.generatedChunks.length + ' (' + this.saveBinary.length + ' / ' + this.maxChunkSize + ')...',
        // });
    ...
    {
        postMessage({
            messageTranslate: 'MAP\\SAVEPARSER\\Generating save file...',
        });
